  ...
  const handleSignIn=async (e)=>{
    ...
    try {
      ...
      navigate("/")
        ...
        setUserData(null)
        setLoading(false)
        setErr(error.response.data.message) // backend se jo error aayega (eg. password incorrect, Internal server error)
    }
  }
    ...
    <div className='w-full h-[100vh] bg-cover flex justify-center items-center' style={{backgroundImage:`url(${bg})`}} >
      ...
      <form className='w-[90%] h-[600px] max-w-[500px] bg-[#00000062] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-[20px] px-[20px]' onSubmit={handleSignIn}>
        ...
        <button className='min-w-[150px] h-[60px] mt-[30px] text-black font-semibold  bg-white rounded-full text-[19px] ' 
          disabled={loading}>{loading?"Loading...":"Sign In"} {/* jab tak loading true h to button disables hoga(to avoid mutiple click) */}
        </button> 

        <p className='text-white mt-[20px] text-[18px]'>Don’t have an account?{" "} <span className='text-blue-400 cursor-pointer' 
